...
const CompletionPreview = ({
  ...
  const fetchStudents = async () => {
    setIsLoading(true);
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/courses/${courseId}/students`, {
  ...
  const removeStudent = async (studentId) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/courses/${courseId}/students/${studentId}`, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
