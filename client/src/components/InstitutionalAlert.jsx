...
const InstitutionalAlert = () => {
  ...
  useEffect(() => {
    ...
    const fetchAlerts = async () => {
      try {
        const { data } = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/notifications`, {
  ...
  const dismissPopup = async (id) => {
    try {
      await axios.put(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/notifications/${id}/dismiss-popup`, {}, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
      setActivePopup(null);
