...
const MonthlyRegister = ({ user, initialSemester, initialCourse, onPersistChange }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const config = { headers: { Authorization: `Bearer ${user.token}` } };
        const studentsRes = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/courses/${selectedCourse.code}/students?semester=${semester}&section=${section}`, config);
        setStudents(studentsRes.data);
        const year = selectedDate.getFullYear();
        const month = selectedDate.getMonth();
        const startDate = new Date(year, month, 1).toISOString();
        const endDate = new Date(year, month + 1, 0, 23, 59, 59).toISOString();
        const attendanceRes = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/attendance/course/${selectedCourse._id}?startDate=${startDate}&endDate=${endDate}&semester=${semester}&section=${section}`, config);
        ...
        if (studentsRes.data.length > 0) {
            ...
            const dailyRes = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/attendance/daily/monthly-bulk?studentIds=${studentIds}&month=${selectedDate.getMonth() + 1}&year=${selectedDate.getFullYear()}`, config);
