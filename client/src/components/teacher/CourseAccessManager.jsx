...
const CourseAccessManager = ({ user, initialSemester, initialCourse, onPersistChange }) => {
  ...
  const fetchAccessData = async () => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/course-access/course/${selectedCourse._id}`, config);
  ...
  const handleFetchHistory = async (student) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/course-access/history/${selectedCourse._id}/${student._id}`, config);
      setHistory(res.data);
  ...
  const handleUpdateAccess = async () => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/course-access/update`, {
        ...
        <div className="flex items-center gap-6">
          ...
          <button 
            onClick={async () => {
              try {
                const config = { headers: { Authorization: `Bearer ${user.token}` } };
                const res = await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/courses/${selectedCourse.code}/auto-restrict`, {}, config);
