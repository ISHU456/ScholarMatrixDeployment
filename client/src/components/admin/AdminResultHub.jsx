...
const AdminResultHub = ({ user }) => {
  ...
  const fetchDepartments = async () => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/departments`);
      setDepartments(data);
    } catch (err) {
  ...
  const fetchSummary = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/results/semester-summary?semester=${semester}&academicYear=${academicYear}&department=${department}&section=${section}`, {
  ...
  const handleUnlockCourse = async (courseId, courseName) => {
    ...
    try {
      setLoading(true);
      const { data } = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/admin/results/unlock`, {
  ...
  const handleCompile = async () => {
    ...
    try {
      setCompiling(true);
      const res = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/results/generate-final`, {
  ...
  const handleApproveCourse = async (courseId, courseName) => {
    ...
    try {
      setLoading(true);
      const { data } = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/admin/results/approve`, {
        courseId, semester, academicYear
      }, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
  ...
  const handleRejectCourse = async (courseId, courseName) => {
    ...
    try {
      setLoading(true);
      const { data } = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/admin/results/reject`, {
  ...
  const handlePublishDeadline = async () => {
    ...
    if (!deadline) return;
    try {
      await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/admin/settings`, {
  ...
  const generateAndSyncTranscript = async (student) => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/results/upload-transcript`, formData, {
        headers: { 
  ...
  const handlePublishSector = async () => {
    ...
    try {
      setPublishing(true);
      await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/results/publish-final`, {
        semester, academicYear, department
      }, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
