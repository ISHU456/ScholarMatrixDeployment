...
const AdminCourseManagement = ({ user }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCourse) {
        await axios.put(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeployment-api.onrender.com'}/api/admin/courses/${editingCourse._id}`, formData, {
          headers: { Authorization: `Bearer ${user.token}` }
        });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeployment-api.onrender.com'}/api/admin/courses/${id}`, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
