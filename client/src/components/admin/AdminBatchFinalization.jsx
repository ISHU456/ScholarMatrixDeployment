...
const AdminBatchFinalization = ({ user }) => {
  ...
  const fetchCourses = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/admin/courses`, {
  ...
  const fetchEligibleStudents = async () => {
    try {
      ...
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/admin/courses/eligible-students?courseId=${selectedCourse._id}&academicYear=${academicYear}`, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
  ...
  const handleFinalize = async () => {
    ...
    try {
        setLoading(true);
        const { data } = await axios.post(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/admin/courses/${selectedCourse._id}/finalize-batch`, {
