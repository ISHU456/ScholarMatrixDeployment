...
const AdminTeacherAttendance = ({ user }) => {
    ...
    const fetchAttendance = async () => {
        setIsLoading(true);
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/admin/teachers/attendance?date=${date}`, {
    ...
    const handleMark = async (teacherId, status, checkIn = '09:00 AM', checkOut = '05:00 PM') => {
        ...
        try {
            await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/admin/teachers/attendance`, {
