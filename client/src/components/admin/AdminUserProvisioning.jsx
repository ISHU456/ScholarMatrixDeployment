...
const AdminUserManagement = ({ user }) => {
    ...
    const fetchUsers = async () => {
        setIsLoading(true);
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeployment-api.onrender.com'}/api/admin/users?role=${role}&dept=${selectedDept}&semester=${selectedSem}&section=${selectedSec}&isActive=${activeStatus}`, {
    ...
    const handleDelete = async (userId) => {
        ...
        try {
            ...
            const response = await axios.delete(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeployment-api.onrender.com'}/api/admin/users/${userId}`, {
                headers: { Authorization: `Bearer ${user.token}` }
            });
    ...
    const handleAuthorize = async (userId) => {
        try {
            await axios.post(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeployment-api.onrender.com'}/api/admin/teachers/pending/${userId}/authorize`, {}, {
                headers: { Authorization: `Bearer ${user.token}` }
            });
            fetchUsers();
        } catch (err) {
    ...
    const handleRoleUpdate = async (userId, newRole) => {
        try {
            await axios.put(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeployment-api.onrender.com'}/api/admin/users/${userId}/role`, { role: newRole }, {
                headers: { Authorization: `Bearer ${user.token}` }
            });
            fetchUsers();
        } catch (err) {
