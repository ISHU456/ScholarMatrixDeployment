...
const ResultEntry = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const config = { headers: { Authorization: `Bearer ${user.token}` } };
        const { data } = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/courses`, config);
  ...
  const handleToggleRowLock = async (resultId, studentId) => {
    ...
    try {
        const { data } = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/results/toggle-lock/${targetResultId}`, {}, { headers: { Authorization: `Bearer ${user.token}` } });
             ...
             {(courseId && currentCourse && !currentCourse.isAuthorized && user.role !== 'admin') ? (
                <div className="absolute inset-0 z-[100] backdrop-blur-xl bg-white/40 dark:bg-[#020617]/40 flex flex-col items-center justify-center p-12 text-center animate-in fade-in duration-500">
                    ...
                    <button 
                      onClick={async () => {
                        try {
                          await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/access-requests`, { courseId }, { headers: { Authorization: `Bearer ${user.token}` } });
