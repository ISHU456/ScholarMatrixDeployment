...
const Courses = () => {
    ...
    const fetchCourses = async () => {
      ...
      try {
        ...
        const [coursesRes, deptsRes] = await Promise.all([
          axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/courses`, {
  ...
  useEffect(() => {
    const fetchAllCounts = async () => {
      ...
      for (const course of courses) {
        try {
          const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/auth/course-activity/${course.code}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setIsSaving(true);
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/admin/courses`, formData, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
      alert('New academic module established.');
      setShowModal(false);
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/courses`, {
