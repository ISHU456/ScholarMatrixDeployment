    ...
    const fetchData = async () => {
      try {
        const [courseRes, annRes] = await Promise.all([
          axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/courses/${courseId}`, {
             headers: { Authorization: `Bearer ${user.token}` }
          }),
          axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/announcements?courseId=${courseId}`)
        ]);
  ...
  const handleUploadCombined = async (e) => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        const res = await axios.post(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/resources/upload?courseId=${courseId}`, formData, {
          headers: { 
        ...
        await axios.post(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/resources?courseId=${courseId}`, {
          title: newTitle, 
          type: newType, 
          fileUrl: newUrl, 
          points: newPoints,
