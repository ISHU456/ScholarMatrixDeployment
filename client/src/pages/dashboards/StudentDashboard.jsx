...
const StudentDashboard = () => {
      ...
      try {
        ...
        await Promise.all(
          studentCourses.map(async (course) => {
            try {
              const rRes = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/resources?courseId=${course.id}`);
  ...
  const fetchLeaderboard = async () => {
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/gamification/leaderboard`, config);
  ...
  useEffect(() => {
    const fetchGamification = async () => {
      try {
        const config = { headers: { Authorization: `Bearer ${user.token}` } };
        const resStats = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/gamification/achievements`, config);
        setGamifiedStats(resStats.data);
        const resQuizzes = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/gamification/quizzes`, config);
  ...
  const fetchClassroomAttendance = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/attendance/classroom`, {
        ...
        {activeQuizId && (
          <QuizArena 
            ...
            onClose={() => {
              ...
              const config = { headers: { Authorization: `Bearer ${user.token}` } };
              axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeploymentserver.onrender.com'}/api/gamification/achievements`, config).then(r => setGamifiedStats(r.data));
            }} 
          />
        )}
      </main>
