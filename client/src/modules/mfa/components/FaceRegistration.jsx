...
const FaceRegistration = () => {
    ...
    const handleRegister = async (passedDescriptors = null) => {
        ...
        setSubmitting(true);
        try {
            await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/mfa/register-face`, 
                { descriptors: finalDescriptors },
                { headers: { Authorization: `Bearer ${user.token}` } }
            );
            
            // Re-fetch user profile to sync all data (including faceRegistered)
            const profileResponse = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/auth/profile`, {
                headers: { Authorization: `Bearer ${user.token}` }
            });
